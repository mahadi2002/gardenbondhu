...
final class SubscriptionRepo
{
    ...
    {
        return Db::first('SELECT * FROM subscriptions WHERE carrier_sub_ref = ?', [$subscriberRef]);
    }
    ...
    {
        return Db::insert(
            'INSERT INTO subscriptions (user_id, carrier_sub_ref, status, daily_amount)
             VALUES (?, ?, "pending", ?)',
    ...
    {
        $sql = 'SELECT s.id, s.user_id, s.carrier_sub_ref, s.status, s.daily_amount,
    ...
    {
        return Db::all(
            'SELECT s.id, s.user_id, s.carrier_sub_ref, s.status, s.daily_amount
               FROM subscriptions s
              WHERE s.status = "pending"
                AND s.carrier_sub_ref IS NOT NULL
