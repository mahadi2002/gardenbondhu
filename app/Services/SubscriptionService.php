...
final class SubscriptionService
{
    ...
    public function startPending(int $userId, string $subscriberRef): int
    {
        ...
        if ($existing !== null && in_array((string) $existing['status'], ['pending', 'active', 'grace'], true)) {
            ...
            if ((string) $existing['carrier_sub_ref'] !== $subscriberRef) {
                Db::exec('UPDATE subscriptions SET carrier_sub_ref = ? WHERE id = ?', [$subscriberRef, $existing['id']]);
            }
            return (int) $existing['id'];
        }

        $id = $this->repo->create($userId, $subscriberRef, (float) config('carrier.amount', 2.78));
    ...
    {
        $hours = (int) config('carrier.period_hours', 24);
    ...
    {
        $hours = (int) config('carrier.grace_hours', 48);
    ...
    public function unsubscribe(int $subscriptionId, string $source = 'user', string $reason = ''): void
    {
        ...
        if ($sub['carrier_sub_ref'] !== null) {
            try {
                GatewayFactory::make()->unsubscribe((string) $sub['carrier_sub_ref']);
            } catch (\Throwable $e) {
