 ...
 *
 * Uses DB_MIGRATE_USER / DB_MIGRATE_PASS when present — the runtime app user
 * has no DDL rights in production, so a SQL-injection bug cannot drop tables.
 */
    ...
    try {
        foreach (splitStatements((string) file_get_contents($file)) as $sql) {
            try {
                ...
                // 005 depends on the ngram parser, which MariaDB does not ship.
                if (str_starts_with($name, '005_') && isIgnorableIndexError($e)) {
                    out('   ! FULLTEXT skipped (' . trim($e->getMessage()) . ') — SearchService will use its LIKE fallback.');
                    continue;
                }
                throw $e;
            }
...
function isIgnorableIndexError(PDOException $e): bool
{
    $msg = strtolower($e->getMessage());
    return str_contains($msg, 'ngram')
        || str_contains($msg, 'parser')
